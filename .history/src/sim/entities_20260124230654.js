...
export function createDwarf(x, y, name = null) {
  ...
  const dwarf = {
    ...
    // === NEW: Unified drives system (replaces old hunger value) ===
    drives: initializeDrives(
      { type: 'dwarf', personality },
      { hunger: 30 } // Start somewhat fed
    ),

    // === NEW: Unified perception system ===
    perceptionRadius: 10,
    recentlyPerceivedEntities: new Map(),
    recentlyPerceivedLocations: new Map(),
    recentlyPerceivedThreat: null,
    lastScentDirection: { dx: 0, dy: 0 },
    ...
    memory: {
      ...
      knownEntities: {},       // IDs of entities met
    },
  ...
  // Initialize perception structures
  initializePerception(dwarf);
