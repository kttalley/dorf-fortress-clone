...
export class ParticleField {
  ...
  update(tick, windAngle, windStrength) {
    ...
    for (const particle of this.particles) {
      ...
      if (gridX >= 0 && gridX < this.width && gridY >= 0 && gridY < this.height) {
        ...
        this.densityField[idx] += particle.life * 0.3;  // Much higher density contribution
      }
    ...
    // Apply diffusion for smooth, fluffy clouds
    // Clouds get many passes for maximum cohesion
    if (this.type.id === 'clouds') {
      this.diffuseDensity();
      this.diffuseDensity();
      this.diffuseDensity();
      this.diffuseDensity();  // Quadruple pass for ultra-cohesive clouds
    } else {
      this.diffuseDensity();
      this.diffuseDensity();  // Double pass for other weather
    }
  }
  ...
  diffuseDensity() {
    ...
    // Clouds use much much stronger diffusion for ultra-fluffy formations
    const diffusionRate = this.type.id === 'clouds' ? 0.50 : 0.15;
...
export const PARTICLE_WEATHER_TYPES = {
  ...
  CLOUDS: {
    ...
    flowScale: 0.008,           // Huge, very slow waves
    flowStrength: 0.08,         // Extremely gentle drifting
    maxParticles: 500,          // Much fuller clouds
  },
  MIST: {
    id: 'mist',
    realm: 'underground',
