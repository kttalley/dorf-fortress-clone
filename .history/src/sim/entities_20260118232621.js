 ...
 * entities.js
 *
 * Canonical entity creation + utilities
 * Integrates deterministic + async LLM name/bio generation
 * Hunger is pressure-based (no death)
 */
...
// =====================================================
// DISPLAY HELPERS (CANONICAL)
// =====================================================

export function getDisplayName(entity) {
  return (
    entity.llm?.nameBio?.name ||
    entity.name ||
    'Unnamed'
  );
}

export function getDisplayBio(entity) {
  return (
    entity.llm?.nameBio?.bio ||
    'A figure yet unrecorded by history.'
  );
}

export function getDominantTraits(entity) {
  if (!entity.personality) return ['balanced'];

  return Object.entries(entity.personality)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .filter(([, v]) => v > 0.6)
    .map(([k]) => k) || ['balanced'];
...
export function createDwarf(x, y, name = null) {
  ...
  // Deterministic immediate name
  generateNameBioSync(dwarf);

  // Async upgrade (fire-and-forget)
...
export function applyFulfillmentDecay(dwarf) {
  ...
  for (const k in dwarf.fulfillment) {
    dwarf.fulfillment[k] = Math.max(0, Math.min(100, dwarf.fulfillment[k]));
  }
}
...
export function createFoodProducer(type, x, y, stock = 40) {
  ...
  generateNameBioSync(producer);
  requestNameBio(producer).catch(() => {});
...
export function createMeetingHall(x, y) {
  ...
  generateNameBioSync(hall);
  requestNameBio(hall).catch(() => {});
