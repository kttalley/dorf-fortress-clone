...
try {
  ...
  // Trigger a UI refresh after names assigned
  if (renderer && statPanel) {
    renderFrame(renderer, document.getElementById('log-entries'));
  }
  ...
  // === NAME GENERATION ===
  try {
    // Wait for LLM batch generation to complete before logging
    await waitForBatchNameGeneration(state.dwarves, 30000);
    state.dwarves.forEach(dwarf => {
      addLog(state, `${getDisplayName(dwarf)} arrives.`);
    });
    ...
    // Partial fallback: log dwarves with whatever names are available
    console.warn('[Init] Name generation timeout:', error.message);
    state.dwarves.forEach(dwarf => {
      if (!dwarf.generatedName || dwarf.generatedName === '...') {
        dwarf.generatedName = generateNameBioLocal(dwarf).name;
