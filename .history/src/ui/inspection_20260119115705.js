...
export function getEntitiesAt(state, x, y) {
  ...
  // Check visitors (external forces)
  for (const visitor of state.visitors || []) {
    if (visitor.x === x && visitor.y === y && visitor.state !== 'dead') {
      entities.push({ type: 'visitor', entity: visitor });
    }
  }
...
export function inspectPosition(state, x, y) {
  ...
  return {
    ...
    hasVisitor: entities.some(e => e.type === 'visitor'),
...
/**
 * Get visitor stats for display panel
 * @param {object} visitor - Visitor entity
 * @returns {object} Formatted stats
 */
export function getVisitorStats(visitor) {
  const raceNames = {
    human: 'Human',
    goblin: 'Goblin',
    elf: 'Elf',
  ...
  const dispositionLabel =
    visitor.disposition > 50
      ? 'Very Friendly'
      : visitor.disposition > 25
      ? 'Friendly'
      : visitor.disposition > 0
      ? 'Neutral'
      : visitor.disposition > -25
      ? 'Hostile'
      : 'Very Hostile';
  ...
  return {
    // Identity
    id: visitor.id,
    name: visitor.generatedName || visitor.name,
    race: raceNames[visitor.race] || visitor.race,
    role: roleNames[visitor.role] || visitor.role,
    bio: visitor.generatedBio || null,

    // Combat stats
    hp: visitor.hp,
    maxHp: visitor.maxHp,
    damage: visitor.damage,
    hpPercent: Math.round((visitor.hp / visitor.maxHp) * 100),

    // Behavioral state
    state: stateNames[visitor.state] || visitor.state,
    disposition: visitor.disposition,
    dispositionLabel,
    satisfaction: Math.round(visitor.satisfaction || 0),
    satisfactionThreshold: visitor.satisfactionThreshold || 100,
...
export function getTooltipLabel(inspection) {
  ...
  if (entities.length > 0) {
    ...
    if (first.type === 'dwarf') {
      return first.entity.generatedName || first.entity.name;
    }
    if (first.type === 'visitor') {
      return first.entity.generatedName || first.entity.name;
    }
    if (first.type === 'food') {
