...
export function updateSidebarThoughts(thoughts = []) {
  ...
  for (const thought of recentThoughts) {
    ...
    // Name with type indicator — always use LLM name if available
    const dwarf = thought.dwarf; // store dwarf reference when pushing thought
    ...
    nameEl.innerHTML = `<span class="thought-icon">${typeIcon}</span> ${displayName}`;
    entry.appendChild(nameEl);

    // Thought text
    const textEl = document.createElement('div');
    textEl.className = 'thought-entry-text';
    textEl.textContent = `"${thought.text || thought.thought}"`; // handle old or new field names
