...
// Extended color palettes for biomes
const BIOME_COLOR_PALETTES = [
  // Cool blues
  { hue: 210, sat: 40, light: 70 },
  // Warm oranges
  { hue: 25, sat: 60, light: 60 },
  // Earthy greens
  { hue: 100, sat: 50, light: 50 },
  // Purple tones
  { hue: 270, sat: 50, light: 60 },
  // Red tones
  { hue: 10, sat: 70, light: 50 },
  // Mint greens
  { hue: 160, sat: 60, light: 65 },
  // Warm yellows
  { hue: 50, sat: 80, light: 55 },
  // Cool purples
  { hue: 280, sat: 40, light: 65 },
  // Ocean blues
  { hue: 200, sat: 70, light: 40 },
  // Sunset oranges
  { hue: 15, sat: 80, light: 55 }
];
...
export function initBiomeTitle(initialName = '') {
  ...
  subtitleEl.id = 'biome-title-subtitle';
  subtitleEl.style.cssText = `
    ...
    margin-top: 4px;
    font-style: italic;
    text-shadow: 0 0 6px rgba(100, 150, 200, 0.2);
  `;
 ...
 * @param {string} subtitle - Optional scenario subtitle
 */

export function updateBiomeTitle(name, colorMod = null, subtitle = '') {
  const nameEl = document.getElementById('biome-title-name');

  const subtitleEl = document.getElementById('biome-title-subtitle');
  ...
  if (colorMod) {
    ...
    // Apply random palette from extended set
    const palette = BIOME_COLOR_PALETTES[Math.floor(Math.random() * BIOME_COLOR_PALETTES.length)];
...
function checkTitleMobileBreakpoint() {
  ...
  if (isMobile) {
    ...
    const subtitleEl = titleEl.querySelector('#biome-title-subtitle');
    if (nameEl) {
