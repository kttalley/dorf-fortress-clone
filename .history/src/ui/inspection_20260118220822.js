...
/**
 * Get formatted tooltip content with close button indicator
 * @param {object} inspection - Result from inspectPosition
 * @param {boolean} showCloseButton - Whether to include close button in content
 * @returns {string} Formatted tooltip content
 */
export function getTooltipContent(inspection, showCloseButton = false) {
  const { entities } = inspection;
  
  let content = '';
  
  // If there's a dwarf, show their stats
  if (entities.length > 0) {
    const firstEntity = entities[0];
    if (firstEntity.type === 'dwarf') {
      const dwarf = firstEntity.entity;
      const stats = getDwarfStats(dwarf);
      
      content = `
${stats.name}
Hunger: ${stats.hunger}/100 (${getHungerDescriptor(stats.hunger)})
Mood: ${stats.mood}/100 (${getMoodDescriptor(stats.mood)})
Wellbeing: ${stats.wellbeing}/100
State: ${stats.state}
...
`;
    ...
    } else if (firstEntity.type === 'food') {
      const food = firstEntity.entity;
      content = `Food Source: ${food.amount} units`;
      if (showCloseButton) {
        content += '\n[Close]';
      }
    }
 ...
 */
export function handleTooltipClose(tooltipId, closeCallback) {
  // This would be called when the close button is clicked
  if (closeCallback) {
    closeCallback(tooltipId);
  }
}
