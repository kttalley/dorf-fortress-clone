...
export function initBiomeTitle(initialName = '') {
  ...
  synopsisEl.id = 'biome-title-synopsis';
  synopsisEl.style.cssText = `
    ...
    font-style: italic;
    opacity: 0.8;
    text-shadow: 0 0 5px rgba(100, 150, 200, 0.2);
    display: block;
  `;
  synopsisEl.textContent = '';
  titleEl.appendChild(synopsisEl);
 ...
 * @param {string} synopsis - Optional scenario synopsis
 */
export function updateBiomeTitle(name, colorMod = null, synopsis = '') {
  const nameEl = document.getElementById('biome-title-name');
  const synopsisEl = document.getElementById('biome-title-synopsis');
  if (!nameEl || !synopsisEl) return;
  ...
  // Ensure synopsis is visible when content exists
  if (synopsis) {
...
function checkTitleMobileBreakpoint() {
  ...
  if (isMobile) {
    ...
    const synopsisEl = titleEl.querySelector('#biome-title-synopsis');
    if (nameEl) {
      nameEl.style.fontSize = '12px';
      nameEl.style.letterSpacing = '1px';
    }
    if (synopsisEl) {
      synopsisEl.style.fontSize = '10px';
      synopsisEl.style.marginTop = '4px';
      synopsisEl.style.display = 'block';
    }
  } else {
    ...
    if (synopsisEl) {
      synopsisEl.style.fontSize = '12px';
      synopsisEl.style.marginTop = '6px';
      synopsisEl.style.display = 'block';
    }
  }
}
