...
export function composeWeatherTile(x, y, terrain, tick, simulator = null) {
  ...
  // Blend foreground color with the weather color
  const weatherFg = blendColors(weather.color, terrain.fg, weather.intensity * 0.9);
  
  // SOLID GREY BACKGROUND for entire tile when weather is present
  // This creates a unified visual appearance across all weather types
  const weatherBg = weather.bgColor || '#444444';
  
  // Brightness: weather intensity makes display brighter/dimmer
  const brightnessShift = (weather.intensity - 0.5) * 20; // -10 to +10
