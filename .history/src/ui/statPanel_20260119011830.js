...
export function createStatPanel(containerEl, gridEl, mapWidth, mapHeight) {
  ...
  return {
    ...
    update(state) {
      ...
      // Only re-render if stats have changed
  const oldName = currentEntity.generatedName || currentEntity.name;
  const newName = dwarf.generatedName || dwarf.name;

  if (oldName !== newName || dwarf.currentThought !== currentEntity.currentThought) {
    currentEntity = dwarf;
