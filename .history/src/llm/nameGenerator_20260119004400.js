  ...
  if (jsonArrayMatch) {
    try {
      ...
      for (const entry of parsedArray) {
        ...
        const bio = entry.bio ?? entry.Bio ?? 'A dwarf of mysterious purpose.';
        if (name && id !== null) {
          const entity = entities.find(e => e.id === id) || entities[id] || entities[0];
          if (entity) results.push({ entity, name, bio });
        }
      }
      if (results.length > 0) {
        console.log(`[Parser] ✓ Extracted ${results.length} entries from JSON array`);
        return results;
      }
    } catch (err) {
      console.log('[Parser] JSON array parse failed:', err.message);
    }
  }

  // Fallback: regex extraction for any "name"
  const nameMatches = [...cleaned.matchAll(/"name"\s*:\s*"([^"]+)"/gi)];
  const bioMatches = [...cleaned.matchAll(/"bio"\s*:\s*"([^"]+)"/gi)];

  for (let i = 0; i < nameMatches.length; i++) {
    const name = nameMatches[i][1];
    const bio = bioMatches[i]?.[1] ?? 'A dwarf of mysterious purpose.';
    const entity = entities[i] || entities[0];
    if (entity) results.push({ entity, name, bio });
  }
...
/**
 * Wait for LLM batch name generation (drop-in replacement)
 * Compatible with old imports from entities.js
 *
 * @param {Array<object>} entities
 * @param {object} worldSnapshot
 * @returns {Promise<Array<object>>} Array of { entity, name, bio, source }
 */
export async function waitForBatchNameGeneration(entities, worldSnapshot = null) {
  // Use the single-batch LLM call if available, otherwise fallback to sequential
  if (llmAvailable) {
    return requestNameBioBatchSingle(entities, worldSnapshot);
  } else {
    return requestNameBioBatch(entities, worldSnapshot);
  }
}
