...
export function parseNameBioResponse(response) {
  if (!response || typeof response !== 'string') {
    console.warn('[Parser] Empty response');
    return null;
  }
  ...
  try {
    ...
    if (typeof parsed.name === 'string' && typeof parsed.bio === 'string') {
      console.log('[Parser] ✓ Direct JSON parse succeeded');
      return {
    ...
    console.log('[Parser] Direct JSON parse failed, trying alternatives...');
    
    // Try to extract JSON from markdown code block
    const jsonMatch = response.match(/```(?:json)?\s*(\{[\s\S]*?\})\s*```/);
    if (jsonMatch) {
      try {
        console.log('[Parser] Trying markdown code block...');
        const parsed = JSON.parse(jsonMatch[1]);
        if (typeof parsed.name === 'string' && typeof parsed.bio === 'string') {
          console.log('[Parser] ✓ Markdown code block parse succeeded');
          return {
            name: sanitizeName(parsed.name),
            bio: sanitizeBio(parsed.bio),
          };
        ...
        console.log('[Parser] Markdown code block parse failed');
      }
    }
    ...
    if (rawMatch) {
      try {
        console.log('[Parser] Trying raw JSON search...');
        const parsed = JSON.parse(rawMatch[0]);
        console.log('[Parser] ✓ Raw JSON search succeeded');
        return {
          name: sanitizeName(parsed.name),
          bio: sanitizeBio(parsed.bio),
        };
      } catch (e3) {
        console.log('[Parser] Raw JSON search failed');
      }
    }

    // Log what we got for debugging
    console.warn('[Parser] Could not parse response. Got:', response.substring(0, 200));
  }
