...
export async function askGame(question, world, history = null) {
  ...
  // Build full prompt
  const userPrompt = buildUserPrompt(worldSummary, question, activeHistory);
...
function analyzeFallbackHunger(dwarves, food) {
  ...
  if (hungriest.hunger > 80) {
    response += `${hungriest.generatedName} is critically hungry (${hungriest.hunger}/100) and at risk of starvation. `;
  } else if (hungriest.hunger > 60) {
    response += `${hungriest.generatedName} is the hungriest (${hungriest.hunger}/100) but not critical. `;
  } else {
    response += `No dwarves are in danger. ${hungriest.generatedName} has the highest hunger at ${hungriest.hunger}/100. `;
  }
...
function analyzeFallbackMood(dwarves) {
  ...
  if (saddest.mood < 30) {
    response += `${saddest.generatedName} has the lowest mood (${saddest.mood}) and may need attention. `;
  } else {
    response += `${saddest.generatedName} has the lowest mood at ${saddest.mood}. `;
  }

  response += `${happiest.generatedName} is the happiest (${happiest.mood}).`;
...
function analyzeFallbackSocial(dwarves) {
  ...
  if (lonely.length > 0) {
    response += `${lonely.map(d => d.generatedName).join(', ')} ${lonely.length === 1 ? 'seems' : 'seem'} lonely (low social fulfillment). `;
  } else {
    response += `No dwarves are critically lonely. `;
  }

  if (socializers.length > 0) {
    response += `Currently socializing: ${socializers.map(d => d.generatedName).join(', ')}.`;
  }
  ...
  for (const d of dwarves) {
    ...
    for (const [otherId, rel] of Object.entries(d.relationships)) {
      ...
      if (other && d.id < other.id && Math.abs(rel.affinity || 0) > 20) {
        pairs.push({ a: d.generatedName, b: other.generatedName, affinity: rel.affinity });
      }
    }
  }
